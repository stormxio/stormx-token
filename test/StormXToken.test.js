...
contract("StormX token test", async function(accounts) {
  ...
  it("revert if not authorized to mint test", async function() {
    await Utils.assertTxFail(stormX.mint(owner, 50, {from: owner}));
  });

  it("revert if calling mint before initialization test", async function() {
    stormX = await StormX.new(reserve, {from: owner});
    await Utils.assertTxFail(stormX.mint(owner, 50, {from: mockSwap}));
  });
